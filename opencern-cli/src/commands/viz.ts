...
export function checkDesktopApp(): boolean {
  if (platform() === 'darwin') {
...
export function openViz(filePath: string, forceBrowser = false): VizResult {
...
export function renderASCII(filePath: string): string[] {
  const lines: string[] = [
    '',
  ...
  try {
    ...
    const events = ((data.events || data.particles || []) as Record<string, unknown>[]).slice(0, 10);
    ...
    if (events.length === 0) lines.push('  No events to display');
  } catch {
