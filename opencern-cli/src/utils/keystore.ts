...
function macSetKey(service: string, key: string): void {
...
function macGetKey(service: string): string | null {
  try {
    const label = `opencern-${service}`;
...
function macDeleteKey(service: string): void {
  try {
    execSync(`security delete-generic-password -s "opencern-${service}" 2>/dev/null`, { stdio: 'ignore' });
  } catch { /* ignore */ }
}

function linuxSetKey(service: string, key: string): void {
  const proc = spawnSync('secret-tool', ['store', '--label', `opencern-${service}`, 'service', 'opencern', 'account', service], {
    input: key,
    encoding: 'utf-8',
  });
  if (proc.status !== 0) throw new Error('secret-tool store failed');
}

function linuxGetKey(service: string): string | null {
  try {
...
function linuxDeleteKey(service: string): void {
  try {
    execSync(`secret-tool clear service opencern account "${service}" 2>/dev/null`, { stdio: 'ignore' });
  } catch { /* ignore */ }
}

export function setKey(service: string, key: string): void {
  if (isMacOS()) {
    try { macSetKey(service, key); return; } catch { /* fallthrough */ }
  }
  if (isLinux() && hasSecretTool()) {
    try { linuxSetKey(service, key); return; } catch { /* fallthrough */ }
  }
...
export function deleteKey(service: string): void {
  if (isMacOS()) macDeleteKey(service);
  if (isLinux() && hasSecretTool()) linuxDeleteKey(service);
...
function fileSetKey(service: string, key: string): void {
  const store = decryptFile();
  store[service] = key;
  encryptFile(store);
}

function fileGetKey(service: string): string | null {
  const store = decryptFile();
  return store[service] || null;
}

function fileDeleteKey(service: string): void {
  const store = decryptFile();
